...
export class SearchableProjectListService {
  ...
  public readonly allProjects$ = combineLatest([
    ...
    switchMap(([projects, enhancePreloadedProjects]:[IProject[],boolean]) => {
      ...
      if(enhancePreloadedProjects && projects.length >= this.preferredPageSize) {
        if(this.preloadProjectIds.length > 0) {
          extraFetches.push(this.fetchProjects([['ancestor', '=', this.preloadProjectIds]]));
        }
        const parents = this.extractParents(projects, this.preloadProjectIds);
        if(parents.length > 0) {
          extraFetches.push(this.fetchProjects([['parent_id', '=', parents]]));
        }
      }
  ...
  private extractAncestors(projects:IProject[]):string[] {
    ...
    // FIXME: Why can't I just call values().filter(...)? According to MDN this is recommended
    // It should be supported since TypeScript 5.6
