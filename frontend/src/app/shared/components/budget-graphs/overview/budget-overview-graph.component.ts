...
import { BudgetEmbeddedGraphComponent, BudgetEmbeddedGraphDataset } from '../embedded/budget-embedded-graph.component';

@Component({
  selector: 'opce-budget-overview-graph',
  templateUrl: './budget-overview-graph.template.html',
  styleUrls: ['./budget-overview-graph.component.sass'],
  changeDetection: ChangeDetectionStrategy.OnPush,
  imports: [BudgetEmbeddedGraphComponent],
  standalone: true,
})
export class BudgetOverviewGraphComponent implements OnInit {
  @Input() initialFilters:any;

  @Input() globalScope:boolean;

  @ViewChild('budgetEmbeddedGraph') private embeddedGraph:BudgetEmbeddedGraphComponent;

  @Input() groupBy = 'status';

  @Input() chartOptions:ChartOptions = { maintainAspectRatio: false };

  public datasets:BudgetEmbeddedGraphDataset[] = [];
  ...
  public sortedDatasets(datasets:BudgetEmbeddedGraphDataset[], params:WpGraphQueryParams[]) {
